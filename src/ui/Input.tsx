...
const FormControl = styled.input<{ $error: boolean, $search: boolean }>`
  ...
  border-style: solid;
  border-color: ${({ $error, $search }) => $search ? 'transparent' : $error ? 'var(--color-danger)' : 'var(--color-greyish)'};
  ...
  &:focus { border-color: var(--color-${props => props.$search ? 'greyish' : 'white'}); }
...
const Input: React.FC<IInput> = ({ error, handler, min, placeholder, search = false, type = 'text' }) => {
  return (
    <Div $search={search}>
      <FormControl
        $error={!!error}
